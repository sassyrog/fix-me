 ...
 * <code>ResultSet</code> to standard out, formatted to look like a table with
 * rows and columns with borders.
 *
 * <p>
 * Stack Overflow website
 * (<a target="_blank" href="http://stackoverflow.com">stackoverflow.com</a>)
 * was the primary source of inspiration and help to put this code together.
 * Especially the questions and answers of the following people were very
 * useful:
 * </p>
 *
 * <p>
 * Question: <a target="_blank" href="http://tinyurl.com/q7lbqeh">How to display
 * or print the contents of a database table as is</a><br>
 * People: sky scraper
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * Question: <a target="_blank" href="http://tinyurl.com/7x9qtyg">Simple way to
 * repeat a string in java</a><br>
...
public class DBTablePrinter {
    ...
    private static class Column {
        ...
        /**
         * Generic SQL type of the column as defined in <a target="_blank" href=
        ...
        /**
         * Generic SQL type name of the column as defined in <a target="_blank" href=
         * "http://docs.oracle.com/javase/8/docs/api/java/sql/Types.html">
         * java.sql.Types </a>.
         */
        ...
        /**
         * Width of the column that will be adjusted according to column label and
        ...
        /**
         * Flag for text justification using <code>String.format</code>. Empty string
         * (<code>""</code>) to justify right, dash (<code>-</code>) to justify left.
         *
         * @see #justifyLeft()
         */
        ...
        /**
         * Column type category. The columns will be categorised according to their
         * column types and specific needs to print them correctly.
         */
        private int typeCategory = 0;

        /**
         * Constructs a new <code>Column</code> with a column label, generic SQL type
         * and type name (as defined in <a target="_blank" href=
         * "http://docs.oracle.com/javase/8/docs/api/java/sql/Types.html">
         * java.sql.Types </a>)
         *
        ...
        /**
         * Adds a <code>String</code> representation (<code>value</code>) of a value to
         * this column object's {@link #values} list. These values will come from each
         * row of a <a target="_blank" href=
         * "http://docs.oracle.com/javase/8/docs/api/java/sql/ResultSet.html"> ResultSet
        ...
        /**
         * Returns the column value at row index <code>i</code>. Note that the index
         * starts at 0 so that <code>getValue(0)</code> will get the value for this
         * column from the first row of a <a target="_blank" href=
        ...
        /**
         * Returns the value of the {@link #justifyFlag}. The column values will be
         * printed using <code>String.format</code> and this flag will be used to right
        ...
        /**
         * Sets {@link #justifyFlag} to <code>"-"</code> so that the column value will
         * be left justified when printed with <code>String.format</code>. Typically
         * numbers will be right justified and text will be left justified.
         */
    ...
    /**
     * Overloaded method that prints rows from table <code>tableName</code> to
     * standard out using the given database connection <code>conn</code>. Total
     * number of rows will be limited to {@link #DEFAULT_MAX_ROWS} and
     * {@link #DEFAULT_MAX_TEXT_COL_WIDTH} will be used to limit the width of text
     * columns (like a <code>VARCHAR</code> column).
     *
     ...
     * Overloaded method that prints rows from table <code>tableName</code> to
     * standard out using the given database connection <code>conn</code>. Total
     * number of rows will be limited to <code>maxRows</code> and
     * {@link #DEFAULT_MAX_TEXT_COL_WIDTH} will be used to limit the width of text
     * columns (like a <code>VARCHAR</code> column).
     *
     * @param conn      Database connection object (java.sql.Connection)
     ...
     * standard out using the given database connection <code>conn</code>. Total
     * number of rows will be limited to <code>maxRows</code> and
     * <code>maxStringColWidth</code> will be used to limit the width of text
     * columns (like a <code>VARCHAR</code> column).
     *
     * @param conn              Database connection object (java.sql.Connection)
     * @param tableName         Name of the database table
    ...
    /**
     * Overloaded method to print rows of a <a target="_blank" href=
     * "http://docs.oracle.com/javase/8/docs/api/java/sql/ResultSet.html">
     * ResultSet</a> to standard out using {@link #DEFAULT_MAX_TEXT_COL_WIDTH} to
     * limit the width of text columns.
     *
     ...
     * Overloaded method to print rows of a <a target="_blank" href=
     * "http://docs.oracle.com/javase/8/docs/api/java/sql/ResultSet.html">
     * ResultSet</a> to standard out using <code>maxStringColWidth</code> to limit
    ...
    public static void printResultSet(ResultSet rs, int maxStringColWidth) {
        try {
            ...
            /*
             * Prepare column labels to print as well as the row separator. It should look
             * something like this: +--------+------------+------------+-----------+ (row
             * separator) | EMP_NO | BIRTH_DATE | FIRST_NAME | LAST_NAME | (labels row)
             * +--------+------------+------------+-----------+ (row separator)
             */
    ...
    /**
     * Takes a generic SQL type and returns the category this type belongs to. Types
     * are categorized according to print formatting needs:
     * <p>
     * Integers should not be truncated so column widths should be adjusted without
     * a column width limit. Text columns should be left justified and can be
     * truncated to a max. column width etc...
     * </p>
     *
     * See also: <a target="_blank" href=
     * "http://docs.oracle.com/javase/8/docs/api/java/sql/Types.html">
     * java.sql.Types</a>
     *
     * @param type Generic SQL type
